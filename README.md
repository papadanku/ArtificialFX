...
This project provides special files to help you create a custom AI assistants for working with ReShadeFX, a tool used to change how games look. The ShaderCell project aims to be a specialised AI assistant for creating ReShadeFX shaders. It does this by using a special collection of information (its "knowledge base") and a technique called Retrieval Augmented Generation (RAG).
...
ShaderCell uses RAG by telling the AI (through `Source/Instructions.md`) to treat the files in the `Source/Knowledge` folder as a primary source of information. When you ask the AI a question or for help with a shader:
...
This RAG approach makes ShaderCell a highly specialized and dependable expert for ReShadeFX shader development, always relying on verified documents instead of just general knowledge.

## Install Instructions (Google Gemini)

This guide helps you get your ShaderCell files ready for use with Google's Gemini platform to create your own specialized AI assistant.

### Step 1: Get the Files

1. Go to the official project page here: **`https://papadanku.github.io/ShaderCell/`**
...
3. **Give Your AI a Name**: Choose a clear name for your new AI, like "ShaderCell Assistant."
...
Use AnythingLLM to run your AI assistant locally and offline. This guide helps you set it up to use ShaderCell's ReShadeFX expertise.
...
### Step 3: Add ShaderCell Knowledge to AnythingLLM

Now, add the ShaderCell knowledge files to your "ReShadeFX Workspace":
...
2. **Drag and Drop**: Drag all files from your ShaderCell project's `Source/Knowledge` folder into the upload area.
...
Finally, instruct AnythingLLM on how to use the ShaderCell knowledge:
...
3. **Copy Instructions**: Navigate to your ShaderCell project folder and open `Source/Instructions.md`. Copy all its content.
...
2. Add the `Source/Knowledge` folder (from where you unzipped the ShaderCell project) to the list of folders GPT4All should look at. This will allow GPT4All to read `Examples.txt`, `REFERENCE.md`, `ReShade.fxh`, and `windows-win32-direct3dhlsl.pdf`.
...
4. In the `System Message` box, copy and paste the entire content of the `Source/Instructions.md` file. This will teach your local GPT4All model to act like the ShaderCell expert, just like the online version.
